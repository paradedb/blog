...
<p align="center">Written by Ming Ying on June 3, 2024</p>

<img src="/images/lakehouse.png" style={{ borderRadius: 10 }} noZoom />

<Note>
  With the release of `v0.10.0`, `pg_analytics` and `pg_lakehouse` have been
  combined into a single extension, `pg_analytics`, that supports fast
  analytical queries over both Postgres tables and data lakes. The new version
  of `pg_analytics` supersedes the version of `pg_lakehouse` described in this
  post. Read the official announcement [here](TODO).
</Note>

With the release of `v0.10.0`, `pg_analytics` and `pg_lakehouse` have been combined
into a single extension, `pg_analytics`, that supports fast analytical queries over
both Postgres and data lakes. The new version of `pg_analytics` supersedes the version
described in this post. Read the official announcement [here](TODO).
...
<img
  src="/images/lakehouse_architecture.png"
