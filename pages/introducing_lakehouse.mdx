...
<img
  src="/images/clickbench_lakehouse_hot.png"
  style={{ padding: 20, background: "#ffffff", borderRadius: 10 }}
  noZoom
/>
...
>
  <tr style={{ display: "flex", color: "#efefef" }}>
    <th
      align="center"
      style={{ width: "100%", textAlign: "left", paddingLeft: 32, fontSize: 16 }}
    >
      Currently Supported
    </th>
  </tr>
  <tr style={{ display: "flex", borderBottom: "0.75px solid #222", color: "#efefef" }}>
    <th
      align="center"
      style={{ width: "34%", textAlign: "left", paddingLeft: 32 }}
    >
      Object Stores
    </th>
    <th
      align="center"
      style={{ width: "33%", textAlign: "left", paddingLeft: 32  }}
    >
      File Formats
    </th>
    <th
  ...
  <tr style={{ display: "flex" }}>
    <td
      align="center"
      style={{ width: "34%", borderBottom: "0.5px solid #1f1f1f", textAlign: "left", paddingLeft: 32 }}
    >
      Amazon S3
    </td>
    <td
      align="center"
      style={{ width: "33%", borderBottom: "0.5px solid #1f1f1f", textAlign: "left", paddingLeft: 32 }}
    >
      Parquet
    </td>
    <td
...
<img src="/images/lakehouse_architecture.png" style={{ borderRadius: 10 }} noZoom />

`pg_lakehouse` uses two Postgres APIs: the executor hook and foreign data wrapper. In Postgres, the executor hook is a function responsible for the actual execution of the query, and
is run immediately after Postgres generates a query plan. `pg_lakehouse` overrides the default Postgres executor hook to reroute foreign table queries to DataFusion, which executes the 
full query and returns rows back to the user.
...
data wrapper is to execute a full scan of the foreign table and send rows to the Postgres query engine.
