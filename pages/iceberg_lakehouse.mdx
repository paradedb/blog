---
title: Putting DuckDB in Postgres to Query Iceberg
---

<p align="center">Written by Ming Ying on June 26, 2024</p>

<img
  src="/images/iceberg_lakehouse_splash.png"
  style={{ borderRadius: 60 }}
  noZoom
/>

Since we launched `pg_lakehouse`, integration with [Apache Iceberg](https://iceberg.apache.org/) has been our most widely requested feature. Today, we’re excited to announce that Iceberg tables are supported. With
`pg_lakehouse` installed, Postgres can directly query Iceberg tables stored in the local file system or in cloud storage.
...
Built for analytics over large datasets, Iceberg is an open-source specification for organizing metadata around files like Parquet. Iceberg metadata allows these files to be treated as
...
<img
  src="/images/duckdb_lakehouse_architecture.png"
